...
async function getGatewayKeypair() {
  ...
  const secretPayload = await getSecret('solana-gateway-key');
  cachedGatewayKeypair = Keypair.fromSecretKey(Uint8Array.from(JSON.parse(secretPayload)));
  return cachedGatewayKeypair;
}

async function getQuickNodeConnection() {
  if (!cachedRpcUrl) {
    cachedRpcUrl = await getSecret('solana-rpc-url');
  }
  return new Connection(cachedRpcUrl.trim(), 'confirmed');
}
...
async function anchorToSolana(payloadHash) {
  ...
  const connection = await getQuickNodeConnection();
...
app.post('/api/v1/compute/dispatch', verifySIWS, async (req, res) => {
  try {
    ...
    const latencyMs = Math.floor(10 + Math.random() * 3);
    ...
    return res.status(200).json({
      ...
      auditProof: {
        network: 'Solana Devnet (via QuickNode)',
        signature: txId,
    ...
    console.error('QuickNode Enterprise Dispatch Error:', err);
